...
def test_strlist(test_num, expected, actual):
    for i in range(len(expected)):
        test_string(f"Part {i} in test {test_num}", expected[i], actual[i])
...
def test_intlist(test_num, expected, actual):
    for i in range(len(expected)):
        test_int(f"Part {i} in test {test_num}", expected[i], actual[i])

# Function calls with the arguments provided 
